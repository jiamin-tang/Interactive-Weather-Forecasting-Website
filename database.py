...
def upsert_forecast_data(df_daily_forecast, df_hourly_forecast):
    ...
    """
    db = client.get_database("weather")
    collection_daily = db.get_collection("daily_forecast_weather")
    collection_hourly = db.get_collection("hourly_forecast_weather")
...
def fetch_forecast_data_as_df():
    ...
    if len(forecast_daily) == 0 or len(forecast_hourly) == 0:
        return None, None
